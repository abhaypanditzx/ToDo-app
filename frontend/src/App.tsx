...
function App() {
  ...
  const submit = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/tasks", { task }); // Send as object
  ...
  const DT = async (id) => {
    const deleted = await axios.delete(`http://localhost:5000/tasks/${id}`)
    setTasks(tasks.filter((prevTask)=> prevTask._id != id))
  }
  return (
    <div className="App">
